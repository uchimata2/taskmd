...
rather than repeating it here keeps that fact in one place (R-1).

`.git` is not a marker: `docs/SCOPE.md` R-9 says nothing in this method assumes version control,
and a project may be a folder on a disk. An environment variable is not one either — a value
somebody has to remember to set is exactly what §1 *Invisibility* rules out.
...
def not_found_message():
    ...
    started would put one machine's disk into output that R-20 requires to be identical on every
    platform, and the reader already knows where they are.
    """
