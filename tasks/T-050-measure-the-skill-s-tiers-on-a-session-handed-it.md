...
| :--- | :--- | :--- |
| 2026-08-21 | (no change) | **§3 step 9's clean-venue condition cannot be met on this machine, and that was not knowable when it was written.** It says a clean measurement needs a project that uses taskmd and does not describe the skill in its always-loaded conventions, treating the mention as this repository's `CLAUDE.md`. [T-175](T-175-observe-whether-the-skill-triggers-in-a-migrated-away-project.md) ran exactly that arrangement on 2026-08-21 — a project with no instruction file of its own, verified — and found this tool named in the machine's **user-level** instruction file, which every project inherits. The confound therefore sits one level above where this record places it, and no choice of project removes it. **Step 9's own result stands**; what has expired is the remedy it names, and this row annotates rather than rewrites it (METHOD rule 5). Whether anything clean is still reachable is [T-205](T-205-decide-whether-a-clean-trigger-observation-is-reachable-on-this-machine.md). |
