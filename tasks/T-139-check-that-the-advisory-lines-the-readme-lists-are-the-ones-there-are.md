...
| Adding a kind is one row, and no test names a member | met | Read off the module: the two kinds differ only in their `Kind` row. No test mentions a command or an advisory by name; every set comes from `cli`. |
