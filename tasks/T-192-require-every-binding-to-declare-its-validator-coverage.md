...
| :--- | :--- | :--- |
| 2026-08-22 | (no change) | **[T-199](T-199-have-an-uninvolved-reader-write-a-coverage-declaration-from-the-clause.md) no longer records this task as its parent** — re-edged to a soft edge by [T-216](T-216-repair-the-three-closed-parents-that-still-have-an-open-child.md), under the rule the owner settled on 2026-08-22 that a child holds every parent open. **§4 is unchanged**, and its closing paragraph *Why this task closes and T-191 did not* already gives the argument this repair formalises. The judgement is in T-216 §3. |
| 2026-08-21 | (no change) | **`check_classes()` moved to `tests/classes.py` the same day**, by [T-197](T-197-derive-the-test-harness-s-problem-class-list-from-the-code.md), which needed the same derivation in `tests/test_cli.py`. §3 says it was added to `tests/test_publishing.py` and that is true of the day it was written; this row is where the present lives (METHOD rule 5). The function is unchanged and that file still imports it, so nothing §3 claims about the check's behaviour is affected. |
