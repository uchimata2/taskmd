...
| :--- | :--- | :--- |
| 2026-08-12 | (no change, closed) | **A third home for this wording was missed and found the same day.** `README.md` gives `CONFIG DRIFT` and `DUPLICATE INDEX` a paragraph each, so shipping `LABEL SHAPE` left it naming two of three. The scope above says *both places an adopter meets it*, and that was true and incomplete: the README is where a **stranger** meets it, before installing anything. Written during the handoff reconcile sweep, and [T-139](T-139-check-that-the-advisory-lines-the-readme-lists-are-the-ones-there-are.md) raised for the guard, because fixing the instance leaves the class as unguarded as it was. |
