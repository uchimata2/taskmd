...
- **The `InstructionsLoaded` hook is installed and unverified, which is a different state from
  ...
  **Annotated 2026-08-17 by [T-171](T-171-test-whether-the-hook-can-see-a-path-scoped-rule.md), which
  was raised to test exactly this: two sentences here have been overtaken, and the decision is
  untouched.** A record now reads something other than `session_start` — `path_glob_match`, written
  when a `.claude/rules/` file was delivered on a read of `CLAUDE.md` — so **the hook does see rule
  loads**, and §3's line deferring the compaction annotation to `load_reason` is confirmed by
  observation rather than left as an assertion nobody had read. The **payload is wider than the seven
  fields listed above**: a rule delivery also carries `prompt_id`, `globs`, and `trigger_file_path`,
  the last naming the file whose read triggered it. Still unobserved: a compaction. This was a
  path-glob load, so what T-169 accepted as a risk is unchanged, and by its own §3 it could not have
  been reversed here anyway.
...
| :--- | :--- | :--- |
| 2026-08-17 | — | **Annotated by [T-171](T-171-test-whether-the-hook-can-see-a-path-scoped-rule.md): §3's sentence about `load_reason` is confirmed, and one statement in §1 is superseded.** T-171 was raised because this record named `load_reason` as the compaction case's future reporter while the rules directory had been empty the whole time the hook was live — so the claim was true of instruction files and merely asserted of rules. It is now observed: a rule delivery writes a line reading `path_glob_match`, and the payload carries three fields §1 does not list. §1's "none reads anything else" is annotated in place rather than edited away. **The decline is untouched and unaffected** — it assumed the worse branch of the compaction question on purpose, and this observation is a path-glob load and not a compaction. |
