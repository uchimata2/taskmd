...
| :--- | :--- | :--- |
| 2026-08-18 | — | **The annotation below has itself gone stale, in under a day, and that is the finding worth more than the correction.** It closes by saying a different probe "now sits in that directory" for [T-171](T-171-test-whether-the-hook-can-see-a-path-scoped-rule.md). T-171 recorded its answer on 2026-08-17 and **deleted** that probe; `.claude/rules/` is now empty. So a row written specifically to repair a false present-tense claim made one of its own, about the same directory, within hours. The cause is not carelessness: the row **stored a fact whose home is elsewhere** — the current contents of a directory this task no longer owns — where it should have pointed. **This record stops tracking `.claude/rules/` here.** Whatever is or is not in it belongs to whichever task holds the live instrument, and T-171 is the last one that did. Nothing below is rewritten; both rows were right on what they knew. |
