...
*Annotated 2026-08-22.* Both children closed, and this task closed with them. The paragraph above is
left as the account of why it waited — which is the thing worth keeping, because the wait was the
rule working rather than an obstacle. Since 2026-08-22 that rule has a home in the shipped method
([`METHOD.md`](../plugin/skills/taskmd/docs/METHOD.md) §4, by
[T-218](T-218-give-the-rule-that-a-child-holds-its-parent-open-a-home-in-the-method.md)) and applies
to every parent rather than only an audit's umbrella, so a later reader does not have to reconstruct
it from `audit.md`.
...
| :--- | :--- | :--- |
| 2026-08-22 | → done | **The umbrella closes on its own stated condition**: both children are `done` — [T-197](T-197-derive-the-test-harness-s-problem-class-list-from-the-code.md) and [T-198](T-198-show-each-quiet-fixture-is-within-its-own-check-s-reach.md), the second closing earlier the same day. §4's fifth row and the paragraph under it are **annotated**, not rewritten (METHOD rule 5); no criterion is re-scored. **One thing a reader should not take from §4 as current:** its first two rows describe the class set as it stood when the audit ran. [T-212](T-212-report-a-closed-parent-that-still-has-an-open-child.md) added `CLOSED PARENT` on 2026-08-22, so the derivation now returns one more than the rows here account for. That is not a gap this audit missed — the new class arrived with two quiet cases in its own fixture and an assertion naming them, which is what a row here would have asked for. The derivation is in `tests/classes.py` and is what to read; no count is maintained here. **Worked under the multi-phase grant recorded at the top of this Log.** |
