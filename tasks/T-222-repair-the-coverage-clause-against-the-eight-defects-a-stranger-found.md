...
| :--- | :--- | :--- |
| 2026-08-22 | (no change) | **Annotation: the grant below was extended and this record was not told.** Later on 2026-08-22 the owner extended the four-task grant to cover **a task raised during the work on the four**. Two were raised and worked to closure under it — [T-227](T-227-the-marked-region-scan-cannot-see-a-class-whose-first-word-is-two-letters.md) and [T-229](T-229-correct-the-migrated-away-fixture-s-own-prose-which-still-says-all-four-commands-refuse.md), each carrying the extension in its own Log — while four others were raised, carried an open question of the owner's, and were left at `specify`. **So the row below reads narrower than what the grant became**, and it is left as written because it records an instruction given at a moment; this row is the annotation METHOD §1 rule 5 asks for instead of a rewrite. **The gap was found hours later, not at the time**, which is the evidence [T-234](T-234-decide-whether-a-grant-s-membership-is-copied-into-every-record-or-derived.md) exists to weigh: two later extensions the same day were annotated within minutes by the session that made them, and this one was not annotated at all until it was looked for. |
| 2026-08-22 | (no change) | **Annotation: a figure this task wrote into the shipped clause was wrong, and [T-227](T-227-the-marked-region-scan-cannot-see-a-class-whose-first-word-is-two-letters.md) found it hours later.** `BINDING.md` §4 was given the sentence *on this validator that is exactly one class*, about the class names the region scan cannot see. It is **two** — `ID WIDTH` and `NO BLOCKER`. **The error is in where the count was taken from, not in the arithmetic**: it was counted against what the two shipped bindings declare, which is a sample, when the question is about the validator's whole class set. The measurement in §3 is right — `github-issues.md` names four and three are read — and the generalisation drawn from it was not. The paragraph in `BINDING.md` is corrected by T-227; this row annotates rather than rewrites, because §3 records what was measured on the day (METHOD §1 rule 5). **T-227 also removed the limit**, so the corrected paragraph says the scan now reads every class name rather than saying how many it misses. |
