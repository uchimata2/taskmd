...
| 2026-08-11 | → proposed | Raised from T-125, which ran the gate before deciding anything and found it red — and then found, from the three existing tags, that it had been red for two releases. Not fixed inside T-125 (METHOD rule 4): that task's job is to ship this tree through the gate, and making the gate run at a different moment is a different outcome with its own cost. Filed `M3` by `tasks/README.md`'s rule — it is new enforcement rather than a correction, so it is outside the standing `M2` authorization and is not started here. `medium` because the thing it protects is the one document a stranger reads before installing, and the failure mode is silence. |
