...
| :--- | :--- | :--- |
| 2026-08-22 | (no change) | **§3 decision 4's first clause is superseded, and the record is annotated rather than edited** (METHOD rule 5). It says the two quiet marks carry no declared value; that was true when written and is now false for one of them — [T-219](T-219-let-a-quiet-mark-declare-a-value-that-begins-with-a-capital.md) repaired the mark syntax the same day, and `T-003`'s mark declares its value. **The rest of decision 4 is unchanged**: assertion 2 still cannot bite on a class that writes task ids bare, and `test_the_two_cases_the_closed_parent_check_must_not_catch` is still what carries the silence. |
