...
| :--- | :--- | :--- |
| 2026-08-21 | (no change) | **The one criterion this task did not meet is now met, by [T-193](T-193-make-the-standing-github-check-fail-before-trusting-it.md).** The procedure has been run four times against a live backlog and made to fail twice. Recorded here rather than edited into §4: that review says truly what was so on the day, and rewriting it would destroy the evidence that the task closed with a known gap - which is the thing that made T-193 get raised. **The first run found the defect in step 3-4's own row 7**, read by shape, 14 spurious failures - so the gap this task recorded was real in a way its author did not expect. |
