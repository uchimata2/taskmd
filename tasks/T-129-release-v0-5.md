...
**Outcome**
The M5 work is published as `0.5.0`: the manifest names it, the gates pass, the tag is annotated,
and the GitHub release says what changed.

**Why this one**
`M2` had no release task until the moment it shipped, and
...
comes last, after M5 is out, so what gets installed on a clean machine is the thing that just
shipped rather than the one before it. So **M5 is not complete when it is tagged.** It is complete
when the published artifact has been proven from outside.

That is a change from `M2`, where the release was the final act and nothing checked the result.
...
**Acceptance criteria**
- [ ] Every M5 task except T-085 is closed when this starts, read with
      `list --work_package M5 --open` rather than from a list
...
| # | Step | Output |
| :-- | :--- | :--- |
| 1 | Read the open M5 set with `list --work_package M5 --open` and confirm only T-085 remains | Criterion 1, or a stop |
...
```text
T-129   planned    M5   plan      Release M5
T-085   proposed   M5   specify   Install the published plugin ...                    blocked
```
...
The leak check then reported two hits in T-020, both a four-part kernel version string. That
...
**Verdict.** All five criteria met. **M5 is not complete**, and that was the point of raising this
...
| :--- | :--- | :--- |
| 2026-08-18 | — | **Typographic only, no claim changed.** This record quoted the four-part kernel version in full while describing the leak check reporting it, which re-tripped the check on every run since. `docs/PUBLISHING.md` §6 already carries the remedy — *elide a component* — and §6's neighbouring rule says to describe a matched line rather than paste it. The string is now described. Applied by [T-183](T-183-decide-what-to-do-about-a-machine-block-already-published-in-t-085.md). |
...
| 2026-08-11 | → proposed | Raised during a handoff, from the maintainer's instruction that T-085 is M5's last item and runs once M5 is released. That instruction needs something to depend on, and a dependency edge needs a task: this is it. **The ordering is the point, not the paperwork.** `0.4.0` shipped with nothing verifying it from outside, and T-085 pointed at whichever version happened to be current when someone got to it. Now it points at this one. Its `blocked_by` carries the edge; nothing here lists what M5 contains. |
