...
| Every fixture out of reach is a child task, and this audit closes only when each resolves | **carried** | One case out of reach → [T-201](T-201-give-the-fenced-table-case-a-row-that-could-be-reported.md); the derivation gap → [T-202](T-202-mark-a-fixture-s-quiet-cases-so-a-sweep-can-find-them.md). Both open, so **this umbrella stays open** — the criterion being met, not deferred. *Annotated 2026-08-22, when this umbrella closed: all four children are now `done` — T-201 and T-202 above, and T-204 and T-210 raised into this umbrella afterwards — so nothing holds it open. The verdict is left as written, because what it was waiting for was the closure and not a different judgement. `taskmd context T-198` is what says so; no list of the four is kept here* |
...
| :--- | :--- | :--- |
| 2026-08-22 | → done | **The umbrella closes, and it closes on its own stated condition.** [`audit.md`](../plugin/skills/taskmd/docs/method/audit.md) step 5 gates an umbrella on its children, and all four are `done`: [T-201](T-201-give-the-fenced-table-case-a-row-that-could-be-reported.md), [T-202](T-202-mark-a-fixture-s-quiet-cases-so-a-sweep-can-find-them.md), [T-204](T-204-count-the-short-row-quiet-case-the-wide-row-audit-left-out.md) and [T-210](T-210-account-for-the-two-derived-fixtures-t-198-s-partition-drops.md). Read from `taskmd context T-198`, which derives the set, rather than from any list. §4's fifth row is **annotated** where it said both children were open — the verdict itself is unchanged, because it already recorded that criterion as met and the closure as the thing waiting (METHOD rule 5). **Criterion 1 remains honestly not met**, carried to T-202, which closed; nothing here re-scores it. Closing this releases [T-191](T-191-audit-whether-each-check-class-has-a-case-it-must-not-catch.md), which it held open as a child. **Worked under the multi-phase grant recorded at the top of this Log.** |
