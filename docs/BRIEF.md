...
`list` is the exception the maintainer argued for. Why grep could not keep the job is written in
that amendment and only there. The non-goal still excludes the query language; it is the *listing*
that was carved out.
